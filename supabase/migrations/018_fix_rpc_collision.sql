-- ============================================
-- FIX: Fulfill Order RPC - Column Name Collision
-- ============================================
-- ISSUE: The 'order_items' table apparently has a 'seller_id' column (NULL), 
-- and 'products' also has 'seller_id'.
-- The previous query 'SELECT oi.*, p.seller_id' caused a collision where 
-- v_item.seller_id resolved to the NULL column from order_items.
--
-- FIX: We alias the product's seller_id to 'product_seller_id' to avoid ambiguity.
...
BEGIN
  ...
  FOR v_item IN 
    -- ALIASING seller_id TO PREVENT COLLISION with potential order_items.seller_id
    SELECT oi.*, p.seller_id as product_seller_id 
    FROM order_items oi
    JOIN products p ON oi.product_id = p.id
    WHERE oi.order_id = p_order_id
  LOOP
    -- Use the ALIASED column
    IF v_item.product_seller_id IS NOT NULL THEN
      -- Double check per item
      IF NOT EXISTS (SELECT 1 FROM seller_earnings WHERE order_item_id = v_item.id) THEN
          ...
          INSERT INTO seller_earnings (
            ...
            v_item.product_seller_id, -- USE ALIAS
            p_order_id, v_item.id, v_item.product_id,
          ...
          SET 
            ...
            updated_at = NOW()
          WHERE id = v_item.product_seller_id; -- USE ALIAS
